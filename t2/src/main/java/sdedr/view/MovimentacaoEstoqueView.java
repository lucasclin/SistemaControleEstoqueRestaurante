 ...
 * (obj.)
 * > Retornar User                       [ok!]
 * > Histórico de movimentação da conta  [ok!]
 * > Realizar uma movimentação por senha [ok!]
 */
...
public class MovimentacaoEstoqueView extends GridPane {
  ...
  private void formularioInsercao(Stage principalStage, CtrlCtrl aux) {
    ...
    precoField.setPromptText("Valor");
    ...
    formularioGrid.add(new Label("Preço:"), 0, 3);
    ...
    inserirButton.setOnAction(e -> {
      ...
      if(e0 != null && e1 != null) {
        ...
        Alert alert = new Alert(Alert.AlertType.WARNING, "ERRO: Somente o Edson pode resolver");
        alert.showAndWait();
        secundarioStage.close();
      }
    });
